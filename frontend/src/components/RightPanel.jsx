...
export default function RightPanel({
  ...
  if (gameData?.moves) {
    ...
    gameData.moves.forEach((move, index) => {
      const moveIndex = index + 1;
      if (move.color === 'white') {
        currentPair = { num: move.number, w: move.notation, wClass: move.classification, wTime: move.time, wIndex: moveIndex, wImportant: isImportantMove(move) };
  ...
  const statRows = [
    { label: 'Brilliant', key: 'brilliant', icon: '!!' }, { label: 'Great', key: 'great', icon: '!' },
    { label: 'Best', key: 'best', icon: '★' }, { label: 'Excellent', key: 'excellent', icon: '👍' },
    { label: 'Good', key: 'good', icon: '✔' }, { label: 'Inaccuracy', key: 'inaccuracy', icon: '?!' },
    { label: 'Mistake', key: 'mistake', icon: '?' }, { label: 'Miss', key: 'miss', icon: '✖' },
    { label: 'Blunder', key: 'blunder', icon: '??' }, { label: 'Book', key: 'book', icon: '📖' },
  ];
  ...
  const renderMoveButton = ({ notation, cls, time, index, important, sideLabel, moveNumber }) => {
    ...
    return (
      <button
        ...
        aria-label={`Move ${moveNumber} ${sideLabel}: ${notation}${bookmarked ? ', bookmarked' : ''}`}
      >
        <div className="move-text">
          {bookmarked ? <span className="bookmark-marker" title="Bookmarked">◆</span> : null}
          {cls ? <span className={`move-class-icon ${CLASS_COLORS[cls?.toLowerCase()] || ''}`} title={cls}>{getIcon(cls)}</span> : null}
          {renderSan(notation, cls, figurineNotation)}
        </div>
        <div className="move-time"><div className="time-bar" /><span>{time || ''}</span></div>
      </button>
    );
  };
    ...
    <div className="right-panel right-panel--motion" aria-busy={loading}>
      <div className="url-input-area review-source-area">
        <div className="panel-heading">
          <div><h1>Game review</h1><p>Load your profile, then open any recent game</p></div>
          <span className={`engine-status ${loading ? 'is-busy' : ''}`}><span /> {loading ? 'Analyzing' : 'Ready'}</span>
        </div>

        <ProfileLoader username={profileUsername} profile={profileData} loading={profileLoading} error={profileError} onLoadProfile={onLoadProfile} onSelectGame={(gameUrl) => fetchGame(gameUrl)} />

        <div className="manual-game-divider"><span>or paste a game link</span></div>
        <form className="url-input-row" onSubmit={(event) => { event.preventDefault(); fetchGame(); }}>
          <input className="url-input" type="url" name="url" autoComplete="off" spellCheck={false} aria-label="Game URL" placeholder="Chess.com or Lichess game URL" value={url} onChange={(event) => setUrl(event.target.value)} />
          <button type="submit" className="analyze-btn" disabled={loading || !url.trim()}>
            {loading ? <Loader2 size={14} className="spin" aria-hidden="true" /> : null}{loading ? 'Loading…' : 'Analyze'}
          </button>
        </form>
      ...
      <div className="engine-info">
        ...
        <span className="engine-name">
          <Cpu size={11} aria-hidden="true" /> {chessEngine === 'stockfish18' ? 'Stockfish 18' : chessEngine === 'stockfish18lite' ? 'Stockfish 18 Lite' : chessEngine === 'torch4' ? 'Torch 4' : chessEngine === 'torch4lite' ? 'Torch 4 Lite' : 'Engine Off'} (Depth {engineDepth || 10})
          {onOpenSettings ? <button type="button" onClick={onOpenSettings} aria-label="Engine settings" className="engine-settings-btn"><Settings size={11} aria-hidden="true" /></button> : null}
        </span>
      </div>

      {gameData?.accuracy ? <div className="accuracy-info"><span style={{ color: '#d4a843', fontVariantNumeric: 'tabular-nums' }}>White Accuracy: {gameData.accuracy.white}%</span><span style={{ color: '#5b7fa6', fontVariantNumeric: 'tabular-nums' }}>Black Accuracy: {gameData.accuracy.black}%</span></div> : null}

      {currentMove ? <MoveStory move={currentMove} moveIndex={currentMoveIndex} bookmarked={bookmarkedSet.has(currentMoveIndex)} onToggleBookmark={onToggleBookmark} onPreviousKey={() => { setIsPlaying(false); if (previousKey) setCurrentMoveIndex(previousKey); }} onNextKey={() => { setIsPlaying(false); if (nextKey) setCurrentMoveIndex(nextKey); }} hasPreviousKey={Boolean(previousKey)} hasNextKey={Boolean(nextKey)} /> : null}

      {activeTab === 'moves' ? (
        <div className="moves-list panel-content" id="moves-panel" role="tabpanel">
          {!gameData ? <div className="empty-state"><div className="empty-state-icon">♞</div><h2>Your review starts here</h2><p>Load your Chess.com profile or paste a game link. Key moments, best-move arrows, and bookmarks appear as you replay.</p></div> : null}
          {movePairs.map((movePair, index) => (
            <div key={index} className="move-row">
              <div className="move-num">{movePair.num}.</div>
              {renderMoveButton({ notation: movePair.w, cls: movePair.wClass, time: movePair.wTime, index: movePair.wIndex, important: movePair.wImportant, sideLabel: 'White', moveNumber: movePair.num })}
              {renderMoveButton({ notation: movePair.b, cls: movePair.bClass, time: movePair.bTime, index: movePair.bIndex, important: movePair.bImportant, sideLabel: 'Black', moveNumber: movePair.num })}
            </div>
          ))}
          {gameData ? <div className="stats-container">{statRows.map((row) => { const white = whiteCounts[row.key]; const black = blackCounts[row.key]; if (white === 0 && black === 0 && row.key !== 'best' && row.key !== 'blunder') return null; return <div key={row.key} className="stat-row" style={{ fontVariantNumeric: 'tabular-nums' }}><div className="stat-label">{row.label}</div><div className={`stat-white ${row.key}`}>{white}</div><div className={`stat-icon ${CLASS_COLORS[row.key]}`}>{row.icon}</div><div className={`stat-black ${row.key}`}>{black}</div></div>; })}</div> : null}
        </div>
      ) : null}

      {activeTab === 'analysis' ? <div className="empty-state panel-content" id="analysis-panel" role="tabpanel"><div className="empty-state-icon">⌁</div><h2>Analysis overview</h2><p>{gameData ? 'Use key-moment navigation to jump between the moves that changed the game.' : 'Analyze a game first to unlock its evaluation.'}</p></div> : null}
      {activeTab === 'openings' ? <div className="empty-state panel-content" id="openings-panel" role="tabpanel"><div className="empty-state-icon">♙</div><h2>Opening explorer</h2><p>{gameData ? 'Opening details appear here when they are available for this game.' : 'Analyze a game to identify its opening.'}</p></div> : null}

      <div className="panel-footer">
        <div className="move-progress" aria-live="polite"><span>Move</span><strong>{currentMoveIndex} / {maxMoves}</strong>{keyMomentIndices.length > 0 ? <small>{keyMomentIndices.length} key moments</small> : null}</div>
        <button type="button" className="review-btn" onClick={() => fetchGame()} disabled={loading || !url.trim()}>{loading ? 'Analyzing game…' : '★ Analyze again'}</button>
        <div className="controls">
          ...
          <div className="controls-main">
            ...
            <button type="button" className={`control-btn play-control ${isPlaying ? 'active' : ''}`} onClick={togglePlay} disabled={!gameData} title={isPlaying ? 'Pause auto-play (Space)' : 'Start auto-play (Space)'} aria-label="Auto play moves">{isPlaying ? <Pause size={16} aria-hidden="true" /> : <Play size={16} aria-hidden="true" />}</button>
