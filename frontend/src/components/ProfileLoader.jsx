...
export default function ProfileLoader({
  ...
  if (!showAccount && (!showRecentGames || !profile?.games?.length)) return null;

  return (
    <section
      className={`profile-loader ${showAccount ? '' : 'profile-loader--recent-only'}`.trim()}
      aria-label={showAccount ? 'Chess.com profile' : 'Recent Chess.com games'}
    >
      {showAccount ? (
        <>
      ...
      {showRecentGames && profile?.games?.length > 0 ? (
