...
const KEY_MOMENT_CLASSES = new Set(Object.keys(CLASS_ARROW_COLORS));
const NEGATIVE_CLASSES = new Set(['inaccuracy', 'mistake', 'miss', 'blunder']);
...
export default function ChessBoard({ fen = 'start', bestMove, playedMove, classification, animationMove, animationDirection = 'forward', isFlipped = false, boardTheme = 'wood', showArrows = true, showCoordinates = true, isJump = false }) {
  ...
  const classKey = String(classification || '').toLowerCase();
  const isKeyMoment = KEY_MOMENT_CLASSES.has(classKey);
  const isNegative = NEGATIVE_CLASSES.has(classKey);
  const playedArrowColor = CLASS_ARROW_COLORS[classKey] || '#60a5fa';
  const showBestArrow = isNegative && bestCoords && bestMove !== playedMove;
  ...
  return (
    <div className="motion-board">
      ...
      {showArrows && isKeyMoment && playedCoords ? (
        <svg className="analysis-arrows" aria-hidden="true">
          ...
          {showBestArrow ? <line className="analysis-arrow analysis-arrow--best" x1={bestCoords.x1} y1={bestCoords.y1} x2={bestCoords.x2} y2={bestCoords.y2} stroke="#4ade80" markerEnd="url(#motion-arrow-best)" pathLength="100" /> : null}
        </svg>
      ) : null}

      {isKeyMoment ? <div className={`board-moment-badge board-moment-badge--${classKey}`}>{classification}</div> : null}
    </div>
  );
}
